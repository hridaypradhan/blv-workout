...
/**
 * usePrototypePoseRuntime React hook
 * 
...
import { PoseRuntimeContract } from "../pose/poseRuntimeTypes";

export interface UsePrototypePoseRuntimeProps {
  currentTimeMs: number;
  activeAnchor: ExerciseTimelineAnchor | null;
  isPlaying: boolean;
}

export function usePrototypePoseRuntime({
  currentTimeMs,
  activeAnchor,
  isPlaying,
}: UsePrototypePoseRuntimeProps): PoseRuntimeContract {
  ...
  const startTracking = () => {
    devLogger.log("Starting prototype pose tracking...");
    setIsReady(true);
    setIsTracking(true);
  ...
  const stopTracking = () => {
    devLogger.log("Stopping prototype pose tracking...");
    setIsReady(false);
    setIsTracking(false);
    setPoseData(null);
    setCurrentAngles({});
  ...
  useEffect(() => {
    if (!isTracking || !activeAnchor || !isPlaying) {
      return;
    }
  ...
  }, [currentTimeMs, activeAnchor, isPlaying, isTracking]);

  return {
    poseData,
    isReady,
    providerSource: "prototype",
    runtimeStatus: isTracking ? "active" : "offline",
    isTracking,
    startTracking,
    stopTracking,
    currentAngles,
    latestRepEvent,
    latestFormError,
    trackingStatusLabel: "Prototype fallback active",
    specificPoseStatus: "prototype_fallback_active",
    poseStatusDetails: {
