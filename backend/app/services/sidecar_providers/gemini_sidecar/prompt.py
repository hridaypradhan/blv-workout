...
def build_gemini_prompt(input_data: SidecarGenerationInput) -> tuple[str, str]:
    ...
    system_instruction = (
        ...
        "For each timeline anchor, generate exercise-specific form model metadata:\n"
        "   - body_region: 'upper_body', 'lower_body', 'core', or 'full_body'\n"
        "   - primary_joints: list of key joint names involved in movement/form (e.g. ['knee_left', 'knee_right'], ['elbow_left', 'elbow_right'])\n"
        "   - counting: counting joint or method (e.g. 'knee_left', 'reps', 'time')\n"
        "   - user_direction: expected user camera orientation ('front_facing' or 'side_facing')\n"
        "   - form_reminders: list of concise, accessible verbal form tips for BLV users (e.g. ['Keep chest proud', 'Weight in heels'])\n"
        "   - form_model: list of FormAngleRatingGemini objects rating key joint angles with joint, importance ('critical', 'important', 'minor', 'ignore'), weight (0.0-1.0), and tolerance_deg (degrees).\n"
