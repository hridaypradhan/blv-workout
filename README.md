...
## AI Sidecar Debugging

To assist developers in validating, quality-checking, and debugging sidecar manifests generated by either the offline prototype or Gemini:

### 1. Enabling Gemini Sidecar Generation
...
### 2. Preprocessing a Video
Submit a YouTube video via the UI or by POSTing to `/api/preprocessing/submit` with a `url`.
To check progress/source, fetch job status via `/api/preprocessing/status/{video_id}`. This response includes:
- `sidecar_provider`: Name of the provider used (`gemini` or `prototype`).
- `caption_status`: Status of caption acquisition (e.g., `captions_found`, `auto_captions_found`, `skipped_offline_prototype`).
- `sidecar_fallback_reason`: In case Gemini failed or was bypassed, the reason is logged here (e.g., `api_key_missing`, `model_error`, `validation_failed`).

### 3. Developer Diagnostics File
When `AI_DIAGNOSTICS_ENABLED=True` is configured in the environment, the backend automatically writes a per-video structured diagnostics JSON summary file under the local prototype data directory:
```
backend/.prototype_data/ai_diagnostics/{video_id}.json
```
This file includes provider details, model configurations, schema versions, caption status, sanitized high-level fallback reasons, manifest list sizes, timeline sanity counts, and a generation timestamp. It never leaks API keys, prompts, transcripts, or raw model responses.

### 4. Manifest Quality Inspection
Call the developer-mode inspection API endpoint:
```http
GET http://localhost:8000/api/preprocessing/manifest/{video_id}/inspection
```
This inspection endpoint is side-effect-free: it loads the manifest from disk persistence and returns a 404 error if none exists. It never invokes manifest generation or saves new files on the fly.

### 5. UI Separation
To maintain clean UX, normal users should not see implementation details. All technical details and debug tags/badges (such as "AI: gemini" or "Captions: auto captions found") are removed from the user-facing Video Library card (`ImportedVideoCard.tsx`). These details are only accessible to developers via the saved diagnostics files and the inspection endpoint.

### 6. Important Reminders
* **Assistant Q&A**: LLM-based interactive coaching is still mocked/deterministic; only sidecar manifest generation uses live Gemini.
* **Gemini Captions Only**: Gemini relies on parsed captions/transcripts rather than direct video/audio files to extract timeline benchmarks.
